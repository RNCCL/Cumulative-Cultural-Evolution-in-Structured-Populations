...
topology_labels = {'ER': 'Random networks', 'BA': 'Scale-free networks', 'WS': 'Small-world networks'}
...
for topology in topologies:
    for transmission_rule in transmission_rules:
        filename = f'{topology}_{transmission_rule}.npz'

        data = np.load(filename, allow_pickle = True)

        average_degrees = list(data['average_degrees'])

        key = (topology, transmission_rule)
...
fig, ax = plt.subplots(1, 3, constrained_layout = True, sharey = True)
...
for j, topology in enumerate(topologies):
    ...
    ax[j].set_title(topology_labels[topology])
